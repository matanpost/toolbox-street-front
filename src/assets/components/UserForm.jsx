...
const UserForm = ( { username, setUsername, email, setEmail, password, setPassword, handleSubmit } ) => {
    ...
    return(
        ...
        <form className="text-3xl font-bold underline" onSubmit={submitCallback}>
            <label>Username<input type="text" value={username} id="username" onChange={event => setUsername(event.target.value)} /></label>
            <label>Email<input type="email" value={email} id="email" onChange={event => setEmail(event.target.value)} /></label>
